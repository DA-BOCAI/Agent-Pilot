...
class TaskControllerTest {
    ...
    void shouldCreateAndPlanTask() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/tasks/{taskId}/workspace", taskId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.taskId").value(taskId))
                .andExpect(jsonPath("$.title").value("build a launch deck"))
                .andExpect(jsonPath("$.status").value("PLANNED"))
                .andExpect(jsonPath("$.displayStatus").value("计划已生成"))
                .andExpect(jsonPath("$.progress.label").value("计划已生成"))
                .andExpect(jsonPath("$.steps[0].code").value("A_CAPTURE"))
                .andExpect(jsonPath("$.steps[0].name").value("理解需求"))
                .andExpect(jsonPath("$.steps[0].action").value("接收并标准化用户意图"))
